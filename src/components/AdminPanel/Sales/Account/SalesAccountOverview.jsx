...
const SalesAccountOverview = () => {
    ...
    const isLoading = allAccountLoading || allAccountTypesLoading || allCompanyTypeLoading || allBrandCatTypeLoading;
    const isError = allAccountError || allAccountTypesError || allCompanyTypeError || allBrandCatTypeError;
    ...
    const ViewSalesAccountColumns = [
        ...
        { key: "website", name: "Website", renderRowCell: (row) => (<a href={row.website} target="_blank" rel="noopener noreferrer">{row.website}</a>), width: 100, sortable: true, showCol: true },
        ...
        { key: "updatedAt", name: "Updated Date", renderRowCell: (row) => DateISOtoNormal(row.updatedAt), width: 100, sortable: true, showCol: true },
        {
            key: "AccountType", name: "Account Type", renderRowCell: (row) => {
                const accountType = allAccountTypes?.find((accountType) => accountType._id === row.account_type_id);
        ...
        },
        {
            key: "company_type_name", name: "Company Name", renderRowCell: (row) => {
                ...
                const companyType = allCompanyType?.find((companyType) => companyType._id === row.company_type_id);
                return companyType ? companyType.company_type_name : 'NA';
            }, width: 100, sortable: true, showCol: true
        },
        {
            key: "Brand_type_name", name: "Brand Category Name", renderRowCell: (row) => {
                const brandType = allBrandCatType?.find((brandCatType) => brandCatType._id === row.category_id);
