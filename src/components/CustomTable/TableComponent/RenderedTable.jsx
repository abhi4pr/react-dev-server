...
const RenderedTable = ({ data, fixedHeader, visibleColumns, rowSelectable, columnsheader, ascFlag, selectAll, currentPage, itemsPerPage, setSelectedRowsIndex, setSelectAll, setSortKey, setAscFlag, setResizing, setSortDirection, resizing, widths, setWidths, setColumns, sortedData, selectedRowsIndex, dataLoading }) => {
    const [preventSelect, setPreventSelect] = useState(false)


    const handleRowSelection = (index) => {
        const actualIndex = (currentPage - 1) * itemsPerPage + index; // Adjust index for pagination
    ...
    return (
        <table className={`${preventSelect ? 'prevent-select ' : ""}`}>
            <thead className={fixedHeader ? 'sticky-header' : ''}>
                <tr >
                    {visibleColumns.some(value => value) && rowSelectable && (<th style={{ width: "40px", height: "50px", display: "flex", justifyContent: "center", alignItems: "center", paddingTop: "4px" }}>
                        <input
                            type="checkbox"
                            checked={selectAll}
                        ...
                        >
                            <div className="table-header">
                                ...
                                <div className="wrapper-filed">
                                    ...
                                    <Dropdown btnHtml={<div className="col-opt"><svg width="15px" height="30px" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" fill="#959DA3" className="bi bi-three-dots-vertical">
            ...
            <tbody>
                {
                    dataLoading ? <div>Loading...</div> :
                        sortedData.length === 0 ? <div className="empty-state">
                            <h3>No Data Found</h3>
                        </div> :
                            ...
                            sortedData.map((row, index) => (
                                ...
                                <tr key={index}>
                                    {visibleColumns.some(value => value) && rowSelectable && (<td style={{ width: "40px", height: "50px", display: "flex", justifyContent: "center", alignItems: "center", paddingTop: "4px" }}>
                                        <input
                                            type="checkbox"
                                            checked={selectedRowsIndex.includes((currentPage - 1) * itemsPerPage + index)}
